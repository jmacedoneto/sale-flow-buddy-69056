...
import { Clock, CheckCircle2, ArrowRight, Phone, Copy, ExternalLink } from 'lucide-react';
...
const statusColumns = [
  ...
  { id: 'concluida', label: 'Concluído', color: 'bg-green-500' },
];
...
const DroppableColumn = ({ id, children }: { id: string; children: React.ReactNode }) => {
  ...
  return (
    <div 
      ref={setNodeRef} 
      className={`bg-muted/30 p-3 rounded-b-lg min-h-[400px] transition-colors ${isOver ? 'bg-primary/10' : ''}`}
    >
      {children}
    </div>
  );
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-1 mt-1">
            ...
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                navigate(`/dashboard?cardId=${atividade.card_id}`);
              }}
            >
...
export const AtividadesAdminKanban = ({ searchTerm, mostrarConcluidas, userId }: AtividadesAdminKanbanProps) => {
  ...
  const getAtividadesByStatus = (status: string) => {
    return atividades.filter(a => {
      if (status === 'pendente') return a.status === 'pendente' || !a.status;
              ...
              <div key={column.id}>
                <div className={`p-3 rounded-t-lg ${column.color}`}>
                  <h3 className="font-semibold text-white">
                    {column.label} ({columnAtividades.length})
                  </h3>
                </div>
